...
class AnomaliFunctionTestCase(unittest.TestCase):
    ...
    def test_environment_cloud_configuration(self):
        """Test cloud environment configuration handling."""
        import os
        import importlib

        # Test dodo-red configuration
        original_cs_cloud = os.environ.get("CS_CLOUD")
        try:
            os.environ["CS_CLOUD"] = "dodo-red"
            importlib.reload(main)
            self.assertEqual(os.environ["CS_CLOUD"], "https://api.dodo.crowdstrike.red")

            # Test dodo-black configuration
            os.environ["CS_CLOUD"] = "dodo-black"
            importlib.reload(main)
            self.assertEqual(os.environ["CS_CLOUD"], "https://api.dodo.crowdstrike.black")

            # Test other values remain unchanged
            os.environ["CS_CLOUD"] = "https://api.crowdstrike.com"
            importlib.reload(main)
            self.assertEqual(os.environ["CS_CLOUD"], "https://api.crowdstrike.com")

        finally:
            # Restore original value
            if original_cs_cloud:
                os.environ["CS_CLOUD"] = original_cs_cloud
            elif "CS_CLOUD" in os.environ:
                del os.environ["CS_CLOUD"]
            importlib.reload(main)
